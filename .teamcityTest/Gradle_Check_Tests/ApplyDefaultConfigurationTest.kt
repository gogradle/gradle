...
class ApplyDefaultConfigurationTest {
    ...
    private
    val failureConditions: FailureConditions = FailureConditions()
    ...
    fun setUp() {
        ...
        val failureConditionsCapturer = slot<FailureConditions.() -> kotlin.Unit>()
        every {
        ...
        every {
            buildType.failureConditions(capture(failureConditionsCapturer))
        } answers {
            failureConditionsCapturer.captured(failureConditions)
            mockk()
        }
    }
    ...
    fun verifyGradleRunnerParams(extraParameters: String, daemon: Boolean, expectedDaemonParam: String) {
        assertFalse(failureConditions.testFailure)

        assertEquals(BuildStep.ExecutionMode.RUN_ON_FAILURE, getGradleStep("GRADLE_RUNNER").executionMode)
